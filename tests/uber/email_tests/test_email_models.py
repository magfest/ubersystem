...
class TestAutomatedEmail(object):
    ...
    def test_reconcile_fixtures(self, automated_email_fixtures):
        ...
        with Session() as session:
            ...
            expected_subject = '{} {} {} {{{{ attendee.full_name }}}}'.format(
                c.EVENT_NAME, c.EVENT_YEAR, c.EPOCH.strftime('%b %Y'))
            for automated_email in session.query(AutomatedEmail):
                assert automated_email.subject == expected_subject
    ...
    def test_render(self, automated_email_fixture):
        with Session() as session:
            ...
            expected = '{} {} {} A Z'.format(c.EVENT_NAME, c.EVENT_YEAR, c.EPOCH.strftime('%b %Y'))
            assert email.render_subject(Attendee(first_name='A', last_name='Z')) == expected
