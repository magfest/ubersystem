...
def generate_barcode_csv(range_start, range_end):
    ...
    for badge_num in range(range_start, range_end+1):
        ...
        if barcode_num in seen_barcodes:
            raise ValueError(
                "COLLISION: generated a badge# that's already "
                "been seen. change barcode key, try again")
...
def generate_barcode_from_badge_num(
        badge_num, event_id=None, salt=None, key=None):
    ...
    if key_len != 10:
        raise ValueError(
            'key length should be exactly 10 bytes, '
            'length={}'.format(key_len))
    ...
    if salted_val > 0xFFFFFF:
        raise ValueError(
            'either badge_number or salt is too large to turn into a '
            'barcode: {}'.format(badge_num))
    ...
    if decrypted_badge_num != badge_num or decrypted_event_id != event_id:
        raise ValueError(
            "internal algorithm error: verification did not decrypt correctly")
...
def get_badge_num_from_barcode(
        barcode_num,
        salt=None,
        key=None,
        event_id=None,
        verify_event_id_matches=True):
...
_valid_base_64_charset = tuple(string.ascii_letters) \
    + tuple(string.digits) + ('+', '/', '=')
...
def assert_is_valid_rams_barcode(barcode):
    if not verify_is_valid_rams_barcode(barcode):
        raise ValueError(
            "barcode validation error: invalid format for RAMS barcode: "
            "'{}'".format(barcode))
...
def _barcode_raw_encrypt(value, key):
    ...
    if len(encrypted_value) != 7:
        raise ValueError(
            "Barcode encryption failure: result should be 7 characters")
...
def _barcode_raw_decrypt(value, key):
    ...
    if len(value) != 7:
        raise ValueError(
            "Barcode decryption failure: barcode should be 7 characters")
    ...
    if value[0] == c.BARCODE_PREFIX_CHAR:
        ...
        raise ValueError(
            "Barcode decryption failure: barcode should start with prefix of "
            "'{}'".format(c.BARCODE_PREFIX_CHAR))
    ...
    try:
        ...
        raise ValueError(
            "Failed to decrypt barcode. check secret_key, event_id, "
            "and whether this barcode is from this event") from e

    if len(decrytped) != 4:
        raise ValueError(
            "Invalid barcode decryption: "
            "output result was not exactly 4 bytes")
