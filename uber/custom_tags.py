    ...
    def notice(self, label, takedown, discount=0, amount_extra=0):
        for day, price in sorted(PRICE_BUMPS.items()):
            if day < takedown and localized_now() < day:
                ...
                return '{} closes at 11:59pm PST on {}. Price goes up to ${} at-door.'.format(label, takedown.strftime('%A, %b %e'), price + amount_extra - discount, (day - timedelta(days=1)).strftime('%A, %b %e'))
