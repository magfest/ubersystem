...
def test_empty_get(mock_apply):
    with Session() as session:
        assert session.attendee({}).paid == c.NOT_PAID  # basic sanity check
        assert mock_apply.called


def test_ignore_csrf(request):
    with Session() as session:
        pytest.raises(Exception, session.attendee, {'paid': c.NEED_NOT_PAY})
        session.attendee({'paid': c.NEED_NOT_PAY}, ignore_csrf=True)
        session.attendee({'paid': c.NEED_NOT_PAY}, allowed=['paid'])
        request.addfinalizer(lambda: setattr(cherrypy.request, 'method', 'GET'))
        cherrypy.request.method = 'POST'
        session.attendee({'paid': c.NEED_NOT_PAY})
