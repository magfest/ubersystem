...
class TestAutomatedEmailCategory:
    ...
    def test_approval_needed_and_we_have_it(self, monkeypatch, set_test_approved_idents, get_test_email_category, incremement_count_unsent_mock):
        job = SendAllAutomatedEmailsJob()
        assert get_test_email_category.is_approved_to_send(daemon_job_results=job)
        assert job.increment_unset_because_unapproved_count.call_count == 0

    def test_approval_needed_and_we_dont_have_it(self, monkeypatch, get_test_email_category, incremement_count_unsent_mock):
        job = SendAllAutomatedEmailsJob()
        assert not get_test_email_category.is_approved_to_send(daemon_job_results=job)
        assert job.increment_unset_because_unapproved_count.call_count == 1

        # attempt to send the same email and we should see the unapproved count go up because it's still unapproved
        assert not get_test_email_category.is_approved_to_send(daemon_job_results=job)
        assert job.increment_unset_because_unapproved_count.call_count == 2
    ...
    def test_should_send_no_email_present(self, monkeypatch, get_test_email_category, attendee1):
        ...
        assert not get_test_email_category._should_send(session=None, model_inst=attendee1, previously_sent_emails={})
    ...
    def test_should_send_already_sent_this_email(self, get_test_email_category, set_test_approved_idents, set_previously_sent_emails_to_attendee1, attendee1):
        ...
        assert not get_test_email_category._should_send(session=None, model_inst=attendee1, previously_sent_emails=previously_sent_emails)
