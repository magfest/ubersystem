    ...
    class SessionMixin:
        def current_admin_account(self):
            try:
                ...
                account_id = None
            if account_id:
                return self.admin_account(account_id)

        def current_supervisor_admin(self):
            try:
                supervisor_id = cherrypy.session.get('kiosk_supervisor_id')
            except AttributeError:
                supervisor_id = None
            if supervisor_id:
                return self.admin_account(supervisor_id)
        ...
        def update_badge(self, attendee):
            ...
            if needs_badge_num(attendee) and num_badges < 2 and (
                    ...
                    or not c.PRINTED_BADGE_DEADLINE or c.BEFORE_PRINTED_BADGE_DEADLINE):
