...
class Reminder:
    ...
    # TODO: need to query shifts as well, for performance
    @classmethod
...
class StopsReminder(Reminder):
    def __init__(self, subject, template, filter, **kwargs):
        Reminder.__init__(self, Attendee, subject, template, lambda a: a.staffing and filter(a), sender=STAFF_EMAIL, **kwargs)

class GuestReminder(Reminder):
    def __init__(self, subject, template, filter=lambda a: True, **kwargs):
        Reminder.__init__(self, Attendee, subject, template, lambda a: a.badge_type == GUEST_BADGE and filter(a), sender=PANELS_EMAIL, **kwargs)

class DeptHeadReminder(Reminder):
    def __init__(self, subject, template, filter=lambda a: True, *, sender=STAFF_EMAIL, **kwargs):
        Reminder.__init__(self, Attendee, subject, template, lambda a: a.ribbon == DEPT_HEAD_RIBBON and len(a.assigned) == 1 and filter(a), sender=sender, **kwargs)

class GroupReminder(Reminder):
    def __init__(self, subject, template, filter, **kwargs):
        Reminder.__init__(self, Group, subject, template, lambda g: not g.is_dealer and filter(g), sender=REGDESK_EMAIL, **kwargs)

class MarketplaceReminder(Reminder):
    def __init__(self, subject, template, filter, **kwargs):
        Reminder.__init__(self, Group, subject, template, lambda g: g.is_dealer and filter(g), sender=MARKETPLACE_EMAIL, **kwargs)
...
Reminder(Attendee, '{EVENT_NAME} payment received', 'payments/attendee_confirmation.html',
         lambda a: a.paid == HAS_PAID)

Reminder(Group, '{EVENT_NAME} group payment received', 'payments/group_confirmation.html',
         lambda g: g.amount_paid == g.total_cost)

Reminder(Attendee, '{EVENT_NAME} group registration confirmed', 'payments/attendee_confirmation.html',
         lambda a: a.group and a != a.group.leader)

Reminder(Attendee, '{EVENT_NAME} extra payment received', 'payments/group_donation.txt',
...
MarketplaceReminder('Your {EVENT_NAME} Dealer registration has been approved', 'dealers/approved.html',
                    lambda g: g.status == APPROVED)

MarketplaceReminder('Reminder to pay for your {EVENT_NAME} Dealer registration', 'dealers/payment_reminder.txt',
                    lambda g: g.status == APPROVED and days_after(30, g.approved) and g.is_unpaid)

MarketplaceReminder('Your {EVENT_NAME} Dealer registration is due in one week', 'dealers/payment_reminder.txt',
                    lambda g: g.status == APPROVED and days_before(7, DEALER_PAYMENT_DUE, 2) and g.is_unpaid)

MarketplaceReminder('Last chance to pay for your {EVENT_NAME} Dealer registration', 'dealers/payment_reminder.txt',
                    lambda g: g.status == APPROVED and days_before(2, DEALER_PAYMENT_DUE) and g.is_unpaid)

MarketplaceReminder('{EVENT_NAME} Dealer waitlist has been exhausted', 'dealers/waitlist_closing.txt',
...
Reminder(Attendee, '{EVENT_NAME} Panelist Badge Confirmation', 'placeholders/panelist.txt',
         lambda a: a.placeholder and a.first_name and a.last_name
                                 and (a.badge_type == GUEST_BADGE or a.ribbon == PANELIST_RIBBON),
         sender = PANELS_EMAIL)

StopsReminder('Want to staff {EVENT_NAME} again?', 'placeholders/imported_volunteer.txt',
              lambda a: a.placeholder and a.staffing and a.registered <= PREREG_OPEN)

StopsReminder('{EVENT_NAME} Volunteer Badge Confirmation', 'placeholders/volunteer.txt',
              lambda a: a.placeholder and a.first_name and a.last_name
                                      and a.registered > PREREG_OPEN)

Reminder(Attendee, '{EVENT_NAME} Badge Confirmation', 'placeholders/regular.txt',
         lambda a: a.placeholder and a.first_name and a.last_name
                                 and a.badge_type not in [GUEST_BADGE, STAFF_BADGE]
                                 and a.ribbon not in [PANELIST_RIBBON, VOLUNTEER_RIBBON])

Reminder(Attendee, '{EVENT_NAME} Badge Confirmation Reminder', 'placeholders/reminder.txt',
         lambda a: days_after(7, a.registered) and a.placeholder and a.first_name and a.last_name)

Reminder(Attendee, 'Last Chance to Accept Your {EVENT_NAME} Badge', 'placeholders/reminder.txt',
...
StopsReminder('{EVENT_NAME} shifts available', 'shifts/created.txt',
              lambda a: state.AFTER_SHIFTS_CREATED and a.takes_shifts)

StopsReminder('Reminder to sign up for {EVENT_NAME} shifts', 'shifts/reminder.txt',
...
StopsReminder('Last chance to sign up for {EVENT_NAME} shifts', 'shifts/reminder.txt',
...
StopsReminder('Still want to volunteer at {EVENT_NAME}?', 'shifts/volunteer_check.txt',
...
StopsReminder('Last chance to personalize your {EVENT_NAME} badge', 'personalized_badges/volunteers.txt',
...
Reminder(Attendee, 'Personalized {EVENT_NAME} badges will be ordered next week', 'personalized_badges/reminder.txt',
...
# Turn these back on after implementing the department head checklist
if False:
    DeptHeadReminder('Last chance for {EVENT_NAME} Department Heads to get Staff badges for your people', 'personalized_badges/dept_head_reminder.txt',
                     lambda a: STAFF_BADGE in PREASSIGNED_BADGE_TYPES and days_before(7, PRINTED_BADGE_DEADLINE))

    DeptHeadReminder('MAGFest Department Ribbons', 'dept_head_ribbons.txt', lambda a: days_before(1, ROOM_DEADLINE),
    ...
    DeptHeadReminder('{EVENT_NAME} staffers need to be marked and rated', 'shifts/postcon_hours.txt',
