...
job_required_role_table = table(
    'job_required_role',
    sa.Column('job_id', sa.Uuid(as_uuid=False), ForeignKey('job.id')),
    sa.Column('dept_role_id', sa.Uuid(as_uuid=False), ForeignKey('dept_role.id')),
)
...
dept_membership_dept_role_table = table(
    ...
    sa.Column('dept_membership_id', sa.Uuid(as_uuid=False), ForeignKey('dept_membership.id')),
    sa.Column('dept_role_id', sa.Uuid(as_uuid=False), ForeignKey('dept_role.id')))
...
def upgrade():
    ...
    sa.Column('dept_membership_id', sa.Uuid(as_uuid=False), nullable=False),
    sa.Column('dept_role_id', sa.Uuid(as_uuid=False), nullable=False),
    ...
    if is_sqlite:
        with op.batch_alter_table('job', reflect_kwargs=sqlite_reflect_kwargs) as batch_op:
            batch_op.add_column(sa.Column('department_id', sa.Uuid(as_uuid=False)))
        ...
        op.add_column('job', sa.Column('department_id', sa.Uuid(as_uuid=False)))
        op.add_column('dept_checklist_item', sa.Column('department_id', sa.Uuid(as_uuid=False)))
