...
def upgrade():
    if is_sqlite:
        with op.batch_alter_table('indie_game', reflect_kwargs=sqlite_reflect_kwargs) as batch_op:
            batch_op.add_column(sa.Column('waitlisted', sideboard.lib.sa.UTCDateTime(), nullable=True))
    else:
        op.add_column('indie_game', sa.Column('waitlisted', sideboard.lib.sa.UTCDateTime(), nullable=True))
