...
def upgrade():
    if is_sqlite:
        with op.batch_alter_table('attendee', reflect_kwargs=sqlite_reflect_kwargs) as batch_op:
            batch_op.add_column(sa.Column('confirmed', sa.DateTime(timezone=True), nullable=True))
    else:
        op.add_column('attendee', sa.Column('confirmed', sa.DateTime(timezone=True), nullable=True))
