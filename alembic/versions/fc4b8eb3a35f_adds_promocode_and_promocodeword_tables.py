...
def upgrade():
    ...
    sa.Column('expiration_date', sideboard.lib.sa.UTCDateTime(), nullable=False),
    ...
    if is_sqlite:
        ...
        with op.batch_alter_table(
            ...
            batch_op.add_column(sa.Column('promo_code_id', sideboard.lib.sa.UUID(), nullable=True))
            batch_op.create_foreign_key(op.f('fk_attendee_promo_code_id_promo_code'), 'promo_code', ['promo_code_id'], ['id'])
    else:
        op.add_column('attendee', sa.Column('promo_code_id', sideboard.lib.sa.UUID(), nullable=True))
